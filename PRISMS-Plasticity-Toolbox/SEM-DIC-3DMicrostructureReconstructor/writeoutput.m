function writeoutput(x,y,z,u,v,w,sc1,sc2)
%Output file name
filename=('T5_hull_3_surf6.vtk');
...
fwrite (fid, reshape(sc1,1,nr_of_elements),'float','b'); %binary data

%append some scalar data
fprintf(fid, '\nSCALARS GB_double float\n'); %ASCII header
fprintf(fid, 'LOOKUP_TABLE default\n'); %ASCII header
fwrite (fid, reshape(sc2,1,nr_of_elements),'float','b'); %binary data
